namespace DnDTool
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            menu.Start( out object? mainMenuOut );
            Console.Clear();
        }
        ...
        static void Travel(Mount mount)
        {
            ...
            if (distanceString != null && int.TryParse(distanceString, out int totalDistance))
            {
                ...
                if (int.TryParse(distanceString, out distance))
                {
                    ...
                    populationMenu.AddItem("Low", null, () => { populationMenu.Stop(); return 1; });
                    populationMenu.AddItem("Medium", null, () => { populationMenu.Stop(); return 2; });
                    populationMenu.AddItem("High", null, () => { populationMenu.Stop(); return 3; });
                    //object populationDensity = 
                    populationMenu.Start( out object? populationDensity );

                    Menu seasonMenu = new Menu("Season");
                    seasonMenu.AddItem("Spring", null, () => { seasonMenu.Stop(); return 1; });
                    seasonMenu.AddItem("Summer", null, () => { seasonMenu.Stop(); return 2; });
                    seasonMenu.AddItem("Autumn", null, () => { seasonMenu.Stop(); return 3; });
                    seasonMenu.AddItem("Winter", null, () => { seasonMenu.Stop(); return 4; });
                    seasonMenu.Start( out object? season );
                    
                    Menu terrainMenu = new Menu("Terrain");
                    terrainMenu.AddItem("Forrest", null, () => { terrainMenu.Stop(); return "Forrest"; });
                    terrainMenu.AddItem("Coastal", null, () => { terrainMenu.Stop(); return "Coastal"; });
                    terrainMenu.AddItem("Desert", null, () => { terrainMenu.Stop(); return "Desert"; });
                    terrainMenu.AddItem("Grassland", null, () => { terrainMenu.Stop(); return "Grassland"; });
                    terrainMenu.AddItem("Hills", null, () => { terrainMenu.Stop(); return "Hills"; });
                    terrainMenu.AddItem("Subterranean", null, () => { terrainMenu.Stop(); return "Subterranean"; });
                    terrainMenu.AddItem("Swamp", null, () => { terrainMenu.Stop(); return "Swamp"; });
                    terrainMenu.AddItem("Mountain", null, () => { terrainMenu.Stop(); return "Mountain"; });
                    terrainMenu.Start(out object? terrain);
                    
                    Menu timeOfDayMenu = new Menu("Time of day");
                    timeOfDayMenu.AddItem("Day", null, () => { timeOfDayMenu.Stop(); return true; });
                    timeOfDayMenu.AddItem("Night", null, () => { timeOfDayMenu.Stop(); return false; });
                    timeOfDayMenu.Start( out object? day );

                    Menu terrainDifficultyMenu = new Menu("Difficult Terrain?");
                    terrainDifficultyMenu.AddItem("Yes", null, () => { terrainDifficultyMenu.Stop(); return true; });
                    terrainDifficultyMenu.AddItem("No", null, () => { terrainDifficultyMenu.Stop(); return false; });
                    terrainDifficultyMenu.Start(out object? difficultTerrain);

                    Console.WriteLine("difficultTerrain = " + difficultTerrain);
                    int movementSpeed = (int)mount / 2;
                    if (difficultTerrain != null && (bool)difficultTerrain)
                    {
